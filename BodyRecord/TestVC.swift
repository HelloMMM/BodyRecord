    ...
    func next() {
        ...
        case 1:
            ...
            UIView.animate(withDuration: 0.5, animations: {
                ...
                self.txt.center = CGPoint(x: self.view.frame.width/2, y: screenSize.height-tabbarVC.tabBar.frame.height-50)
                self.txt.text = "首次使用, 需設定資訊\n請點擊下方按鈕"
                UIView.animate(withDuration: 0.5) {
                    self.txt.alpha = 1
                }
            })
            
            spotlightView.move(Spotlight.Oval(center: CGPoint(x: screenSize.width/2, y: screenSize.height-tabbarVC.tabBar.frame.height+16), diameter: 60), moveType: .disappear)
        case 2:
